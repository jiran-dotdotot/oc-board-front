...
- 예시 도메인: `src/components/board/`(`listData.ts` · `constants.ts` · `BoardListScreen.tsx` · `PostDetailScreen.tsx`) + 라우트 `src/routes/board.$boardId.tsx`·`post.$postId.tsx`.
  목록·상세는 `@tanstack/react-query` + `apiClient`(`src/services/postService.ts`) 실연동. 폼은 react-hook-form + zod(`zodResolver`) — `WriteScreen.tsx`(글쓰기·수정, `writePayload.ts` 순수 빌더 + `tests/writePayload.test.ts`)가 실연동 예시.
