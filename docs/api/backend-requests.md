...
> | **신설 [BR-037](#br-037)**          | 게시글 첨부 업로드 **실연동**(실소스로 계약 확인, `POST /posts/{id}/attachments`) · 대표이미지 업로드는 서버 생성 경로가 없어 여전히 막힘 · 복사본 문서 backfill 남음                                                                                                                                                            |
>
...
| [BR-011](#br-011) | 관리자의 타인 게시글 삭제 정책 확인                | 해소(관리자 허용 확인)   | 게시글 상세 관리자 삭제            |
...
| [BR-022](#br-022) | 파일 복원 시 직접 해제한 북마크 보존               | 열림(전제 정정)          | 휴지통 복원·북마크                 |
...
| [BR-027](#br-027) | 카테고리 삭제와 자식 생성의 snapshot 경합 방지     | 열림(경합만·격하)        | 카테고리 동시 생성·삭제            |
| [BR-028](#br-028) | 공개 API 주소·활성 설정·토큰 TTL의 배포값 확인     | 열림                     | 로그인·API 주소·세션 갱신          |
| [BR-029](#br-029) | 브라우저 S3 업로드·다운로드와 CDN 접근 환경 확인   | 열림(설계 정당·인프라)   | S3 PUT/GET·CDN 이미지              |
...
| [BR-036](#br-036) | member 토큰 획득·수명·갱신·클레임·등급 확인        | 대부분 해소(잔여 1건)    | 비번변경·MFA 화면(정본 없음)       |
| [BR-037](#br-037) | 첨부 업로드 실연동 · 대표이미지는 생성 경로 없음   | 첨부 해소 · 썸네일 열림   | 게시글 첨부(붙임)·대표이미지(막힘) |
...
## 2026-09-10 — 실소스(oc-api-go · oc-api-laravel) 재검증

지금까지 열린 26건을 **복사본 `docs/api/go/` 가 아니라 옆 저장소의 실제 소스**로 다시 확인했다
(`../oc-api-go/internal/**`, `../oc-api-laravel/**`). 대장 3건의 전제가 틀렸음을 확인해 정정하고,
일부를 격하하며, BR-037 은 이미 구현돼 있음을 확인한다. 아래가 **현행 판정**이고, 각 BR 절의 옛 서술보다 이 표가 우선한다.

| BR      | 판정                                                                           | 실소스 근거(actual source)                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------- | ------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **008** | 프론트 무관·배포 확인                                                          | 저장 200 ≠ 배달. 프론트는 「알림 감」을 표시하지 않고 표시할 수단도 없어야 정상. 운영 워커·브로커 확인만 남음                                                                                                                                                                                                                                                                                                                                             |
| **011** | **대장 전제 틀림 → 해소.** 관리자 삭제는 «허용»된다                            | 실 SQL `postwritequery.go:338-352` = 작성자 ∨ 회사관리자(`permissionquery.go:70-73`) ∨ 게시판관리자. Go 주석 「작성자뿐」·복사문서가 틀렸다. purge(영구삭제)만 작성자 전용(`postwritequery.go:399-404`). 프론트는 `is_mine \|\| is_admin` 로 이미 맞다. **별건**: 삭제 성공 뒤 `removeQueries` 가 활성 상세 쿼리를 재조회시켜 404(라우터 navigate 가 비동기라 언마운트 전 재조회) — 프론트 소소 버그                                                      |
| **018** | 진짜 갭(확인)                                                                  | `management/setting.go:268-269` `latest_post_day`/`type` 에 minimum·enum 태그 없음 → DB CHECK(`migrations/board/000001_initial_schema.sql:913-915`) 위반이 500. 400 이어야 함                                                                                                                                                                                                                                                                             |
| **019** | 진짜 갭(확인). 음수 통과 = 예                                                  | `board/driveupload.go:213` `size` 에 `minimum:0` 없음(주석 `:190-207` 「size HAS NO FLOOR」). 0은 의도, 음수는 DB CHECK(`size>=0`) → 배치 전체 500. v1 파리티 주석                                                                                                                                                                                                                                                                                        |
| **020** | 진짜 갭(확인)                                                                  | presign 게이트 `driveuploadquery.go:123-127` 에 `deleted_at IS NULL` 없음 → 죽은 폴더로 업로드 성공. 폴더 감지 토스트는 무관(아래 별도 설명)                                                                                                                                                                                                                                                                                                              |
| **021** | 진짜 갭(확인·저우선)                                                           | `folderquery.go:254` `parent_id = COALESCE(@parent_id, parent_id)` → null=유지. 루트 이동 방법 없음(주석 `:230-234` 미구현 명시). 프론트에 이동 UI 자체가 없음                                                                                                                                                                                                                                                                                            |
| **022** | **대장 전제 틀림 → 정정.** 자료실 삭제는 즉시삭제가 아니라 soft-delete(휴지통) | `drivefilewrite.go:120` `TrashDriveFiles` = `deleted_at` 기록, 복원 `drivefilewritequery.go:489`. 물리삭제는 별도 워커 30일(`drivepurge.go:126,152`). **백엔드 갭 아님.** 다만 «자료실 화면에 휴지통 탭이 없어» 사용자에겐 사라져 보임 → UX 과제. 복원은 내 활동 휴지통에 있음                                                                                                                                                                            |
| **023** | 경합은 유효·복원 실재                                                          | `RestoreDriveFiles`(`drivefilewrite.go:251`) 실재하므로 BR-022 의 「복원 없음」 전제는 무효. 정리·복원 경합 자체는 남음                                                                                                                                                                                                                                                                                                                                   |
| **024** | 수정 요청 유지(사용자 결정)                                                    | 게시글 복원이 사전 개별삭제 댓글까지 활성화 — 백엔드 수정 요청으로 둔다                                                                                                                                                                                                                                                                                                                                                                                   |
| **025** | 진짜 갭(확인) — 서버가 purge 본문을 작성자에게 줌                              | `postdetailquery.go:187-192` 가시성 게이트가 소유자면 통과·`purged_at` 미검사. 프론트 모달은 **표시만** 가림(`state='DEL'`→`blockedKind`). 서버 레벨 갭 유지                                                                                                                                                                                                                                                                                              |
| **026** | 진짜 갭(확인)                                                                  | `platform/webhook/client.go:143-161` non-2xx 를 에러로 안 봄 → `worker/alarmdelivery.go:241-262` 가 반송돼도 `is_notice_alarm/is_send_alarm=true` 잠금. v1 파리티 주석                                                                                                                                                                                                                                                                                    |
| **027** | **대장 과장 → 격하.** 캐스케이드는 있다                                        | `board.go:417-462` 카테고리 삭제가 하위 게시판·글·첨부·자료폴더를 전부 soft-delete. 남는 건 「삭제 도중 동시 생성」 경합 하나(`categorydelete.go:92-122`) — 희귀 엣지                                                                                                                                                                                                                                                                                     |
| **028** | 배포 확인(보류)                                                                | 실주소·TTL·프록시 한도는 배포값. 개발 단계 무관                                                                                                                                                                                                                                                                                                                                                                                                           |
| **029** | 설계 정당 → 격하                                                               | 수십 GB 를 서버 경유 없이 브라우저가 직접 S3 PUT/GET 하는 presign 은 올바른 설계. presign 을 바꾸자는 요청이 아니라 배포 시 버킷 CORS·IAM·CDN 이 우리 origin 으로 열렸는지 인프라 확인만                                                                                                                                                                                                                                                                  |
| **032** | 대장 정확 — 정확히 2개 API                                                     | `is_not_paging=1` 에 배열을 주는 건 `GET .../posts`(`postlist.go:234`)·`GET .../drive-files`(`drivefile.go:523`) 둘뿐. `/mine`·`/bookmarks` 는 봉투 유지. 프론트는 어디서도 안 보냄                                                                                                                                                                                                                                                                       |
| **033** | 진짜 갭(확인)                                                                  | 파일 DTO `drivefile.go:362-405` 에 `drive_folder_id`(UUID)만, 폴더 이름·경로 없음. 로드되는 관계는 `Board{id,title}` 뿐                                                                                                                                                                                                                                                                                                                                   |
| **034** | 진짜 갭(확인)                                                                  | `postlistfilter.go:393` `"%"+v+"%"`, ESCAPE 절 0건 → `%`·`_` 가 wildcard 로 과다매칭(주입 아님, 바인드 파라미터). v1 파리티 주석                                                                                                                                                                                                                                                                                                                          |
| **035** | 확인 위치 정리                                                                 | 읽기 `GET /board/me`→`company_user_setting.recent_search_keyword`, 지우기 member 전용 PUT(이 앱 401). 프론트는 localStorage `searchWords`(상한 8) 정본. 토큰 주체 불일치가 원 요청                                                                                                                                                                                                                                                                        |
| **036** | **대부분 해소.** TTL·갱신·클레임·등급 확인(Laravel)                            | access 2h(관리자는 exp 없음·20분 서버세션), refresh 7일(iat 기준). refresh 는 옛 access 를 헤더 필수(`AccountController.php:1111`) — 우리 인터셉터가 붙여 정상, `authority` 는 서버가 무시. 등급 차단은 회사티어가 아니라 사용자 좌석 라이선스 403(`:730-745`). 클레임 `company_id/user_id`(int)·`sub='Authorization'`·`scopes:[ROLE_MEMBER]`. **잔여**: 로그인 응답의 `is_required_password_change`·`mfa_*` 를 프론트가 안 읽음(정본에 화면 없음 → 후속) |
| **037** | **서버 구현됨 → 재분류.** 문서만 부재                                          | `attachmentupload.go` 실재: `POST /posts/{id}/attachments` multipart `file`(1개), 0<size≤100MB(`:57`), 확장자 필수, 이미지 4천만px 이하, 응답 `{id,state:ACTIVE}`(`:103-104`). 썸네일 `drivethumbnail.go` = `GET /drive-files/{id}/thumbnail-url`→`{url}` presign. **프론트 실연동 가능**, 백엔드엔 문서 backfill 만 요청                                                                                                                                 |
| **038** | 대장 정확 유지                                                                 | 사용자별 댓글 수 집계 없음(`/me` 카운터 0·`comments/mine` 없음). 집계 엔드포인트 요청                                                                                                                                                                                                                                                                                                                                                                     |
| **039** | 자료실만 30일(사용자 결정)                                                     | 자료 purge 워커 30일 실재(`drivepurge.go:126`), 게시글은 물리삭제 워커 없음 → 게시글 휴지통엔 보관 안내 붙이지 않음(이미 그러함)                                                                                                                                                                                                                                                                                                                          |
| **040** | 전체 탭 없음(사용자 결정)                                                      | 글+자료 병합 목록 API 없음 → 하위탭 게시글·자료 2종만                                                                                                                                                                                                                                                                                                                                                                                                     |
| **041** | 진짜 갭(확인)                                                                  | `postlist.go:416-426` `/posts/mine` 의 `state` 에 enum 없음 → 오타가 400 아니라 200 빈 페이지. (URL id 케이스는 별개 — 없는 글은 404→「삭제되었거나 존재하지 않는」이 정상, 깨진 id 만 「불러오지 못했습니다」)                                                                                                                                                                                                                                           |
| **042** | 대장 정확 유지                                                                 | 작성·수정 응답 `postwrite.go:521-584` 에 관계 없음 → 저장 후 상세 1회 재조회, ACT 상세 GET 이 작성자 제외 없이 조회수+1(`postdetail.go:106-113`·`postdetailquery.go:586-595`)                                                                                                                                                                                                                                                                             |
| **043** | 유지                                                                           | 본문 이미지는 나모 호스트 저장(Go 밖). 저장 버튼마다 `POST namo/.../upload/save` 로 본문 사본도 감                                                                                                                                                                                                                                                                                                                                                        |

**BR-020 프론트 동작(질문 답):** 「삭제된 폴더입니다」 토스트는 **업로드가 아니라 폴더 이동에서** 뜬다.
서버는 없는/삭제된 `drive_folder_id` 에 에러 대신 `path=[]`(브레드크럼 없음)를 준다. 프론트는 폴더 id 가 있는데
브레드크럼이 비면(`DriveScreen.tsx:450` `ghostFolder = folderId && drive && crumbs.length===0`) 「유령 폴더」로 보고
토스트(`drive-folder-gone`) 후 루트로 `replace` 이동한다(`:451-458`). 업로드 경로는 이와 **독립**이라 삭제된
폴더로도 presign→S3 PUT→완료가 그대로 성공한다(그래서 BR-020 이 실제 갭). 감지는 «폴더를 열 때», 업로드는
«id 를 이미 쥔 채» 진행돼 서로 만나지 않는다.
...
- 상태: **첨부 업로드 = 해소(실연동)** · 대표이미지 업로드 = 열림(서버 생성 경로 부재) · 문서 backfill = 열림
- 발견일 / 발견 맥락: 2026-09-09 / [스테일 재대조](backend-replies/staleness-ebff9af.md) §4 와 `routes.txt` 대조. **분류: 계약 누락(라우트는 등록, 문서 미작성). 갱신: 2026-09-10, 실소스(`../oc-api-go`) 재검증 → 첨부 업로드 실연동. 이력: 신설(열림) → 첨부 해소.**
...
- 프론트 임시 조치: **구현하지 않는다.** 자료실 썸네일은 현재 `VITE_S3_FILE_BASE_URL` 기반 표시를 유지하고, 게시글 첨부는 기존 업로드 경로를 유지한다. 두 신규 경로를 호출하는 코드는 없다.
  **갱신 2026-09-10(글쓰기 Go 연동)**: 게시글 첨부·대표 이미지 **업로드 UI 는 정본대로 그렸지만 호출은 게이트로 막았다** — `POST_ATTACHMENT_UPLOAD_ENABLED=false` 한 곳. 계약이 있는 **삭제**(`delete_file_id[]`·`delete_thumbnail_id[]`, `06-post-write.md:283-284`)는 수정 모드에서 구현했다.

  **갱신 2026-09-10(실소스 재검증 → 첨부 업로드 실연동)**: 옆 저장소 실소스 `../oc-api-go` 에서 계약을 직접 읽어 **첨부 업로드를 붙였다.** 근거·확정 사항:
  - **첨부 업로드는 구현돼 있다.** `POST {S}/posts/{id}/attachments`, multipart 필드 **`file` 단일·필수**, 서버 검증 = 확장자 필수·`0<size≤100MB`·이미지면 `W*H≤4천만px`, 응답 `{id, state:"ACTIVE"}`(`internal/transport/httpapi/board/attachmentupload.go:26-105` · 라우트 `routes.go:38`). 권한 게이트는 **작성자 + 쓰기 권한**(아니면 404/403). **글 id 가 있어야** 부른다.
  - 프론트 배선: 저장(생성/수정)으로 글 id 를 얻은 **뒤** 대기 파일을 **한 건씩 순차** 업로드한다([`uploadPostAttachments`](../../src/services/postService.ts) → [`WriteScreen.save`](../../src/components/board/WriteScreen.tsx)). 본문·제목이 먼저 저장되므로 첨부 일부가 실패해도 글은 유실되지 않고, 실패분은 화면에 남겨 재시도한다. [`POST_ATTACHMENT_UPLOAD_ENABLED`](../../src/components/board/constants.ts) 를 `true` 로 올렸다. **여전히 남는 backend 요청은 «복사본 `docs/api/go` 문서 backfill»뿐이다**(계약은 실소스로 확인, 복사본엔 아직 없음).
  - **대표 이미지 «업로드»는 서버에 생성 경로가 없다 → 별도 게이트로 유지**([`POST_THUMBNAIL_UPLOAD_ENABLED=false`](../../src/components/board/constants.ts)). 유일한 업로드 경로가 INSERT 시 **`type='FILE'` 하드코딩**이고(`attachmentupload.go:36`), 전용 썸네일 업로드 라우트가 없다. 수정 핸들러도 `file`·`thumbnail` multipart 키를 **create 는 400·update 는 무시**로 거부한다(`internal/transport/httpapi/board/postwrite.go:393-407` — "This service has no upload path", B-10). 삭제만 계약이 있어(`delete_thumbnail_id` → v11 병합표라 `delete_file_id` 로 합쳐짐, `postwrite.go:440-452`) 기존 썸네일 제거는 게이트 밖이다.
- 게시글 첨부 요구사항(글쓰기 화면이 필요로 하는 것 — 레거시 실측 + 디자인 정본 기준, **요청안이며 현행 계약이 아니다**):
  ...
  4. **응답** — 현재 업로드 응답은 `{id, state:"ACTIVE"}` 뿐이라 `origin_file_name`·`size` 가 없어 프론트가 방금 올린 File 메타로 목록 행을 만든다. `files[]` 항목(`05-post-read.md:116-128`)과 같은 모양(`id`·`origin_file_name`·`size`)을 돌려주면 그 임시 조립이 사라진다. **대표 이미지**: 실소스 확인 결과 업로드는 항상 `type='FILE'` 로만 넣고(`attachmentupload.go:36`) 썸네일 생성 경로가 없다 — 업로드가 **`type`(FILE/THUMBNAIL)을 받거나** 전용 라우트를 열어 달라. 그래야 정본의 「대표 이미지」 박스를 붙일 수 있다.
