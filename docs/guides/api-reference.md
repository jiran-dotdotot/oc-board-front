...
| board | `/api/v1/board`의 면제 외 경로 | Go 발급 HS256 bearer, issuer `oc-api-go/board`, exp 필수. OfficeWave 토큰 fallback 없음 |
...
GET S/attachments/{id}/download-url 또는 S/drive-files/{id}/download-url로 `{url,origin_file_name,expires_at}`을 받는다. 기본5분 presigned GET URL을 그대로 사용하며 board 토큰을 S3에 붙이거나 src/object key로 URL을 재조립하지 않는다. 발급은 bytes 전송·HEAD 검사가 아니므로200 뒤 실제 GET이 실패할 수 있다.
...
| 항목 | 확인 위치·조치 |
| --- | --- |
| 공개 Base URL·활성 설정·실제 TTL | 배포 origin·프록시, BOARD_*/OFFICENEXT_*. localhost·기본TTL을 운영 상수로 만들지 않음 |
