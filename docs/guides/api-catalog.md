...
> ## 🚨 이 색인은 스테일이다 (2026-09-09)
>
> 기준 커밋은 `65b7f49`, **서버는 `ebff9af`** 다. 아래 67개 색인은 다음만큼 틀렸다:
>
> | 구분               | 내용                                                                                                                                                      |
> | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
> | **삭제 3**         | `POST /api/v1/board/token` · `/login` · `/refresh` — **서버에 없다.** 아래 「01 인증·유저」 3행을 쓰지 마라                                               |
> | **이사 6**         | `/api/v1/companies/{c}/{departments,admins,settings…}` → **`/api/v1/board/companies/{c}/…`**. **`user_id` 세그먼트 없음**, 본인은 `/users/me` 리터럴      |
> | **신규 2**         | `GET {S}/drive-files/{id}/thumbnail-url` · `POST {S}/posts/{id}/attachments` — 계약 문서 없음, 붙이지 않는다([BR-037](../api/backend-requests.md#br-037)) |
> | **인증 라벨 전건** | `board` 계약이 **폐지**됐다. 게시판 표면 64경로는 전부 `member`(OfficeWave ES256). 아래 「인증」 열의 `board` 표기는 전부 무효                            |
> | 전수               | 89 → **88**(exempt 10 / member 64 / service 14)                                                                                                           |
>
> **URL·파라미터·응답 계약 자체는 그대로다**(🟡 6파일 38엔드포인트). 즉 이 색인은 «경로 찾기»에는
> 계속 쓸 수 있고, **인증·자격증명 부분만 무효**다. 근거·실측표: [backend-replies/](../api/backend-replies/README.md).
> **전면 재작성은 백엔드가 새 사본을 전달한 뒤에 한 번에 한다**(두 번 고치지 않기).
> member 토큰 획득 경로는 아직 미정이다([BR-036](../api/backend-requests.md#br-036)).
...
| 항목               | 현재 Go 계약·확인 위치                                                                                                                                                                                                                                                                        |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 인증               | ⚠️ **`ebff9af` 에서 `board` 계약이 폐지됐다** — 게시판 표면은 전부 `member`(OfficeWave ES256 + `ROLE_MEMBER`)다. `exempt` 는 10개뿐이고 **로그인·토큰 교환·refresh 경로가 하나도 없다**. 아래 표의 `board` 라벨과 `exempt` 자격증명 서술은 무효다. [실측표](../api/backend-replies/README.md) |
...
| ------------ | ---------------------------- | ----------------------------------------------- | -------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------- |
| 01 인증·유저 | `POST /api/v1/board/token`   | 🚫 **삭제됨(ebff9af)** — OfficeWave 토큰 교환   | `exempt` | 본문 member JWT 검증; 관리자 동기화·refresh 저장도 수행                      | [01:218](../api/go/01-auth-user.md#post-apiv1boardtoken)   |
| 01 인증·유저 | `POST /api/v1/board/login`   | 🚫 **삭제됨(ebff9af)** — OfficeNext 계정 로그인 | `exempt` | board 토큰만 반환; 상위 status와 로컬 오류를 구분                            | [01:274](../api/go/01-auth-user.md#post-apiv1boardlogin)   |
| 01 인증·유저 | `POST /api/v1/board/refresh` | 🚫 **삭제됨(ebff9af)** — board 토큰 갱신        | `exempt` | refresh 일회 소비 후 새 발급; 응답유실·후속 실패에도 재사용 불가             | [01:334](../api/go/01-auth-user.md#post-apiv1boardrefresh) |
...
| 도메인            | METHOD · 전체 경로                                                                                      | 기능                      | 인증     | 주의                                                                 | 원문                                                                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------------- | ------------------------- | -------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| 02 관리·설정·정렬 | `POST /api/v1/board/companies/{company_id}/admins` ↗ **이사(ebff9af)**                                  | 회사 관리자 추가          | `member` | 회사 관리자만; 중복403, 삭제 관리자 복원 응답 created_at은 현재시각  | [02:121](../api/go/02-management.md#post-apiv1companiescompany_idadmins)                               |
| 02 관리·설정·정렬 | `POST /api/v1/board/companies/{company_id}/settings` ↗ **이사(ebff9af)**                                | 회사 설정 생성            | `member` | 회사 관리자만; 중복은500, 기본 사용자 설정도 함께 생성               | [02:180](../api/go/02-management.md#post-apiv1companiescompany_idsettings)                             |
| 02 관리·설정·정렬 | `PATCH /api/v1/board/companies/{company_id}/settings` ↗ **이사(ebff9af)**                               | 회사 설정·메인 위젯 수정  | `member` | 회사 관리자만; edit_company_main_board는 JSON 배열을 인코딩한 문자열 | [02:226](../api/go/02-management.md#patch-apiv1companiescompany_idsettings)                            |
| 02 관리·설정·정렬 | `PATCH /api/v1/board/companies/{company_id}/settings/users/me` ↗ **이사(ebff9af)**                      | 본인 알림 설정 수정       | `member` | 회사 설정 없으면404; {}도 설정 생성·복원 가능                        | [02:295](../api/go/02-management.md#patch-apiv1companiescompany_idsettingsusersme)                     |
| 02 관리·설정·정렬 | `PUT /api/v1/board/companies/{company_id}/settings/users/me/recent-search-keywords` ↗ **이사(ebff9af)** | 최근 검색어 전체 교체     | `member` | 키 생략/null은 전체 삭제; 앞8개만 저장, live 사용자 설정 없으면404   | [02:347](../api/go/02-management.md#put-apiv1companiescompany_idsettingsusersmerecent-search-keywords) |
...
| 10 업로드·조직도·이미지·버전 | `GET /api/v1/board/companies/{company_id}/departments` ↗ **이사(ebff9af)**                   | 회사 조직도 조회                 | `member` | 루트 없으면200 빈 Body; category 필터의 빈 허용집합은 전체 members                   | [10:271](../api/go/10-upload-department-client.md#get-apiv1companiescompany_iddepartments)                                    |
