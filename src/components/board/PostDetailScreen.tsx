...
import { usePostDetail, usePostDetailMutations } from '@/hooks/usePostDetail'
...
export function PostDetailScreen() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (dl.state.open) return setDlCancelAsk(true)
    }
    document.addEventListener('keydown', onKey)
    return () => document.removeEventListener('keydown', onKey)
  }, [delComment, delOpen, dlCancelAsk, dl.state.open])
  ...
  const download = async (files: PostFile[]) => {
    ...
    if (r === 'unavailable') showToast(t('detail-dl-unavailable'), 'error')
    ...
    <div className="flex w-full flex-col gap-2.5">
      ...
      <article className="flex flex-col bg-card">
        ...
        <header className="flex flex-col gap-3.5 border-b border-gray-200 px-0.5 pt-2.5 pb-[18px] min-[631px]:px-1 min-[631px]:pb-5">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <span
              className={`inline-flex size-[34px] flex-none items-center justify-center rounded-full text-sm font-bold text-on-pastel ${pastel(author ?? '?')}`}
            >
              {initial(author)}
            </span>
            <div className="flex min-w-0 flex-col gap-px">
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs text-gray-400">
                ...
                <button
                  ...
                  aria-label={t('detail-history-open')}
            ...
            <div className="ml-auto flex flex-none gap-0.5 print:hidden">
              ...
              {/* 수정은 «작성자 본인만» — 관리자도 서버가 403 을 준다(docs/api/06:169) */}
              {post.is_mine && (
                <Link
                  to="/write"
                  search={{ postId }}
                  ...
                  className={ICON_BTN}
                >
                  <PencilIcon className="size-[15px] flex-none" />
                </Link>
              )}
        ...
        <div className="flex flex-col gap-3.5 px-0.5 pt-5 pb-4 min-[631px]:px-1 min-[631px]:pt-6 min-[631px]:pb-[18px]">
          ...
          <PostAttachments files={attachments} onDownload={download} configured />
        ...
        <ConfirmModal
          ...
          onConfirm={() =>
            m.removePost.mutate(undefined, {
              ...
              onError: () => {
                setDelOpen(false)
                showToast(t('detail-delete-failed'), 'error')
              },
            })
          }
        />
      ...
      {dlCancelAsk && (
        <ConfirmModal
          ...
          sub={t('drive-dl-progress-n', {
            done: dl.state.done,
            total: dl.state.total,
            percent: dl.state.percent,
          })}
      ...
      {dl.state.open && (
        <Modal onClose={() => setDlCancelAsk(true)} label={t('drive-dl-title')} role="alertdialog">
          ...
          <div className="flex w-[340px] max-w-full flex-col gap-3.5 rounded-md bg-card px-5 pt-[22px] pb-[18px] shadow-[var(--shadow-modal)]">
            ...
            <div
              role="progressbar"
              aria-valuenow={dl.state.percent}
              aria-valuemin={0}
              aria-valuemax={100}
              className="h-1.5 w-full overflow-hidden rounded-full bg-gray-100"
            >
              <div className="h-full bg-primary" style={{ width: `${dl.state.percent}%` }} />
            </div>
            <span aria-live="polite" className="text-s text-gray-500">
              {t('drive-dl-progress-n', {
                done: dl.state.done,
                total: dl.state.total,
                percent: dl.state.percent,
              })}
