 ...
 *  - 404: 라우트 모델 바인딩 실패(없거나 hard delete) — docs/api/05-post-read.md:200
 *  - 200 + `state='DEL'`: 휴지통으로 옮겨진 글. soft delete 가 아니라 상태 전환이라
 *    라우트 바인딩은 통과하고 200 이 온다(docs/api/06-post-write.md:51) → 응답을 보고 걸러야 한다.
...
export function listPage(rowNum: number | null | undefined, limit: number): number | undefined {
  // 공지글은 row_num 이 1 이고, state != ACT 이면 null 이다(docs/api/05-post-read.md:239)
